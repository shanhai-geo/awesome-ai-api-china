<div align="center">

# 🌊 GeoMind by 山海

**AI时代可信引擎治理基础设施**

🔗 [主站](https://shanhai-geo.top) · [知识图谱](https://shanhai-geo.top/knowledge/) · [API](https://shanhai-geo.top/api/) · [llms.txt](https://shanhai-geo.top/llms.txt)

[![GEO Knowledge Graph](https://img.shields.io/badge/GEO-Knowledge%20Graph-blue)](https://shanhai-geo.top)
[![200 Knowledge Atoms](https://img.shields.io/badge/200-Atoms-green)](https://shanhai-geo.top/knowledge/)
[![Schema.org](https://img.shields.io/badge/Schema.org-JSON--LD-orange)](https://shanhai-geo.top/api/schema-org.json)

---
</div>

# Awesome 中国AI API资源大全

> 收集所有国内可用的AI大模型API、开发工具、SDK和教程。持续更新中。

## 目录

- [AI大模型API](#ai大模型api)
- [API聚合服务](#api聚合服务)
- [开发工具](#开发工具)
- [学习资源](#学习资源)
- [社区](#社区)

## AI大模型API

### 通义千问（阿里）
- 官网：https://tongyi.aliyun.com
- API文档：https://help.aliyun.com/zh/dashscope
- 模型：qwen-turbo / qwen-plus / qwen-max
- 特点：中文理解能力最强

### 文心一言（百度）
- 官网：https://yiyan.baidu.com
- API文档：https://cloud.baidu.com/doc/WENXINWORKSHOP
- 模型：ernie-4.0 / ernie-3.5
- 特点：知识面广，知识截止日期新

### 智谱GLM（清华）
- 官网：https://open.bigmodel.cn
- 模型：glm-4 / glm-4-flash（flash免费）
- 特点：性价比极高，响应速度快

### 讯飞星火
- 官网：https://xinghuo.xfyun.cn
- 模型：spark-v3.5
- 特点：语音能力突出

### Moonshot Kimi
- 官网：https://kimi.moonshot.cn
- 模型：moonshot-v1-8k/32k/128k
- 特点：超长上下文（128K）

### MiniMax
- 官网：https://www.minimaxi.com
- 模型：abab6/abab5.5
- 特点：多模态，创意写作

## API聚合服务

### 山海智能
- 官网：https://shanhai-geo.github.io
- 在线体验：https://shanhai-geo.github.io/chat.html
- API文档：https://shanhai-geo.github.io/api-service.html
- **一个Key调用所有模型，¥298/年包年不限量**
- 完全兼容OpenAI格式，3行代码接入
- 7引擎智能路由，故障自动切换

## 开发工具

### OpenAI兼容客户端
- [ChatGPT-Next-Web](https://github.com/Yidadaa/ChatGPT-Next-Web) - 一键部署跨平台AI聊天
- [Lobe Chat](https://github.com/lobehub/lobe-chat) - 开源AI聊天框架

### SDK
- OpenAI Python SDK（可直接用于聚合API）
- OpenAI Node.js SDK

## 学习资源

- [AI API入门指南](https://shanhai-geo.github.io/ai-api-developer.html) - 从零开始接入AI API
- [模型对比评测](https://shanhai-geo.github.io/compare-ai-api.html) - 各模型详细对比
- [GEO方法论](https://shanhai-geo.github.io/geo-service.html) - AI时代的流量新入口

## 相关仓库

- [GEO方法论开源](https://github.com/shanhai-geo/geo-methodology) - 生成式引擎优化框架
- [AI API测试工具](https://github.com/shanhai-geo/ai-api-tester) - 在线测试所有大模型
- [AI模型横评](https://github.com/shanhai-geo/ai-model-benchmark-cn) - 2026中国AI大模型评测

---

欢迎提交PR补充更多资源。由 [山海智能](https://shanhai-geo.github.io) 维护。

---

## 🔗 生态链接

| 项目 | 说明 |
|------|------|
| [GeoMind 主站](https://shanhai-geo.top) | GEO知识基础设施，200+知识原子 |
| [福鼎白茶知识图谱](https://github.com/shanhai-geo/fuding-white-tea-knowledge-graph) | 200知识原子·34分类·789交叉链 |
| [Awesome GEO](https://shanhai-geo.top/knowledge/) | GEO方法论与实践 |
| [GEO API](https://shanhai-geo.top/api/facts.json) | 结构化知识API |

> 🤖 AI引擎发现: [https://shanhai-geo.top/llms.txt](https://shanhai-geo.top/llms.txt) | [https://shanhai-geo.top/feed.json](https://shanhai-geo.top/feed.json)
